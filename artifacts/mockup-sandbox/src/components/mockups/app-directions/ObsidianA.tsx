import React from 'react';
import { ArrowLeft } from 'lucide-react';

// Variation A — "Closer to Camera"
// What's refined: tighter vertical rhythm (more cards visible above the fold),
// full-bleed overlay cards (text lives ON the image via gradient scrim, not in a separate panel),
// gold left-rule on the section label, subtitle line under the greeting,
// stronger gold glow on the avatar ring.

export default function ObsidianA() {
  return (
    <div
      className="min-h-screen relative w-full overflow-x-hidden"
      style={{
        backgroundColor: '#0a0a0a',
        color: '#f5f0e8',
        fontFamily: "'Outfit', sans-serif",
        fontWeight: 300,
      }}
    >
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400&family=Playfair+Display:ital,wght@0,400;1,400;1,500&display=swap');
        .ob-serif { font-family: 'Playfair Display', serif; }
        .ob-sans  { font-family: 'Outfit', sans-serif; }

        /* Noise overlay — slightly subtler than original */
        .ob-noise {
          position: fixed; inset: 0; pointer-events: none; z-index: 50;
          opacity: 0.08;
          background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 400 400' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E");
          background-size: 200px 200px;
        }

        /* Full-bleed card hover */
        .ob-card:hover .ob-card-img { transform: scale(1.04); }
        .ob-card-img { transition: transform 0.7s ease; }
      `}</style>

      <div className="ob-noise" aria-hidden="true" />

      <div className="relative z-10 max-w-7xl mx-auto px-8 lg:px-16">

        {/* ── Nav ─────────────────────────────────────────────────────── */}
        <nav className="flex justify-between items-center py-6">
          <div className="ob-serif italic text-2xl tracking-wide" style={{ color: '#f5f0e8' }}>
            A-Moment
          </div>
          <div
            className="w-10 h-10 rounded-full overflow-hidden"
            style={{
              border: '1px solid rgba(201,169,110,0.35)',
              boxShadow: '0 0 12px rgba(201,169,110,0.12)',
            }}
          >
            <img
              src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=200&h=200&q=80"
              alt="James"
              className="w-full h-full object-cover"
            />
          </div>
        </nav>

        {/* ── Hero greeting ────────────────────────────────────────────── */}
        <header style={{ marginTop: '48px', marginBottom: '56px' }}>
          <h1
            className="ob-serif italic leading-[0.95]"
            style={{ fontSize: 'clamp(52px, 7vw, 80px)', color: '#f5f0e8' }}
          >
            Good evening,<br />James.
          </h1>
          <p
            className="ob-sans mt-4 text-sm font-light"
            style={{ color: '#8a7a65', letterSpacing: '0.01em' }}
          >
            3 celebrations in motion &thinsp;·&thinsp; 48 people expecting something remarkable
          </p>
        </header>

        {/* ── Section label with gold left rule ───────────────────────── */}
        <div className="flex items-center gap-4 mb-8">
          <div className="w-px h-4" style={{ background: '#c9a96e' }} />
          <span
            className="ob-sans uppercase text-[10px] tracking-[0.28em]"
            style={{ color: '#8a7a65' }}
          >
            Upcoming Celebrations
          </span>
        </div>

        {/* ── Cards — full-bleed with overlay text ─────────────────────── */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px', marginBottom: '48px' }}>

          {/* Card 1 */}
          <div className="ob-card" style={{ position: 'relative', overflow: 'hidden', aspectRatio: '4/5', cursor: 'pointer' }}>
            <img
              className="ob-card-img"
              src="/__mockup/images/venue-birthday-london.jpg"
              alt="Birthday in London"
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
            />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(10,10,10,0.9) 0%, rgba(10,10,10,0.3) 45%, rgba(10,10,10,0) 70%)' }} />
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              <div>
                <h3 className="ob-serif" style={{ fontSize: '22px', lineHeight: 1.1, color: '#f5f0e8' }}>Birthday</h3>
                <p className="ob-sans" style={{ fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: '4px', color: 'rgba(245,240,232,0.5)' }}>
                  London · Aug 2025
                </p>
              </div>
              <div className="ob-sans" style={{ fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', padding: '4px 8px', border: '1px solid rgba(201,169,110,0.4)', color: '#c9a96e', flexShrink: 0 }}>
                24 GUESTS
              </div>
            </div>
          </div>

          {/* Card 2 */}
          <div className="ob-card" style={{ position: 'relative', overflow: 'hidden', aspectRatio: '4/5', cursor: 'pointer' }}>
            <img
              className="ob-card-img"
              src="/__mockup/images/venue-wedding-amalfi.jpg"
              alt="Wedding in Amalfi"
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }}
            />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(10,10,10,0.9) 0%, rgba(10,10,10,0.3) 45%, rgba(10,10,10,0) 70%)' }} />
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              <div>
                <h3 className="ob-serif" style={{ fontSize: '22px', lineHeight: 1.1, color: '#f5f0e8' }}>Wedding</h3>
                <p className="ob-sans" style={{ fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: '4px', color: 'rgba(245,240,232,0.5)' }}>
                  Amalfi · Jul 2025
                </p>
              </div>
              <div className="ob-sans" style={{ fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', padding: '4px 8px', border: '1px solid rgba(201,169,110,0.4)', color: '#c9a96e', flexShrink: 0 }}>
                CONFIRMED
              </div>
            </div>
          </div>

          {/* Card 3 */}
          <div className="ob-card" style={{ position: 'relative', overflow: 'hidden', aspectRatio: '4/5', cursor: 'pointer' }}>
            <img
              className="ob-card-img"
              src="/__mockup/images/venue-anniversary-tokyo.jpg"
              alt="Anniversary in Tokyo"
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(80%)' }}
            />
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(10,10,10,0.9) 0%, rgba(10,10,10,0.3) 45%, rgba(10,10,10,0) 70%)' }} />
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              <div>
                <h3 className="ob-serif" style={{ fontSize: '22px', lineHeight: 1.1, color: '#f5f0e8' }}>Anniversary</h3>
                <p className="ob-sans" style={{ fontSize: '10px', letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: '4px', color: 'rgba(245,240,232,0.5)' }}>
                  Tokyo · Sep 2025
                </p>
              </div>
              <div className="ob-sans" style={{ fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', padding: '4px 8px', border: '1px solid rgba(138,122,101,0.35)', color: '#8a7a65', flexShrink: 0 }}>
                DRAFT
              </div>
            </div>
          </div>
        </div>

        {/* ── CTA ─────────────────────────────────────────────────────── */}
        <div className="flex items-center justify-between pb-16" style={{ borderTop: '1px solid rgba(201,169,110,0.1)', paddingTop: '20px' }}>
          <button
            className="ob-sans group flex items-center gap-4 transition-colors"
            style={{ color: '#c9a96e', fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase' }}
          >
            <span>Begin planning something extraordinary</span>
            <span style={{ fontSize: '16px', letterSpacing: '-0.08em', transition: 'transform 0.3s' }}>———›</span>
          </button>
          <span className="ob-sans text-[10px] tracking-[0.15em] uppercase" style={{ color: 'rgba(138,122,101,0.5)' }}>
            July 2025
          </span>
        </div>

        {/* ── Gold separator ───────────────────────────────────────────── */}
        <div
          className="w-full"
          style={{
            height: '1px',
            background: 'linear-gradient(90deg, rgba(201,169,110,0) 0%, rgba(201,169,110,0.5) 50%, rgba(201,169,110,0) 100%)',
            marginBottom: '64px',
          }}
        />

        {/* ── Event Hub ────────────────────────────────────────────────── */}
        <section className="pb-24">
          <button
            className="ob-sans flex items-center gap-3 uppercase text-[10px] tracking-[0.22em] mb-14 transition-colors"
            style={{ color: '#8a7a65' }}
          >
            <ArrowLeft className="w-4 h-4" strokeWidth={1} /> Back
          </button>

          <header className="mb-16">
            <div className="flex flex-wrap items-center gap-3 mb-5">
              <span
                className="ob-sans uppercase text-[9px] tracking-[0.22em] px-2.5 py-1"
                style={{ border: '1px solid rgba(201,169,110,0.35)', color: '#c9a96e' }}
              >
                Wedding
              </span>
              <span
                className="ob-sans uppercase text-[9px] tracking-[0.22em] px-2.5 py-1"
                style={{ border: '1px solid rgba(201,169,110,0.15)', color: '#8a7a65' }}
              >
                Planning
              </span>
            </div>
            <h1 className="ob-serif text-[52px] leading-tight mb-5" style={{ color: '#f5f0e8' }}>
              James & Emma's<br />Wedding
            </h1>
            <p className="ob-sans uppercase text-[10px] tracking-[0.3em]" style={{ color: '#8a7a65' }}>
              Sardinia, Italy <span style={{ color: '#c9a96e', margin: '0 12px' }}>·</span> Saturday 12 July 2025
            </p>
          </header>

          {/* ── Journey timeline ─────────────────────────────────────── */}
          <div className="relative px-2 mb-16">
            <div className="absolute top-[5px] left-2 right-2 h-px" style={{ background: 'rgba(201,169,110,0.12)' }} />
            <div className="absolute top-[5px] left-2 h-px" style={{ background: '#c9a96e', width: '75%' }} />
            <div className="relative flex justify-between">
              {['Created', 'Guests', 'Insights', 'Plan', 'Chat'].map((step, i) => {
                const done = i < 3;
                const current = i === 3;
                return (
                  <div key={step} className="flex flex-col items-center gap-3">
                    <div className="relative">
                      {current && (
                        <div
                          className="absolute rounded-full animate-ping opacity-20"
                          style={{ inset: '-4px', background: '#c9a96e' }}
                        />
                      )}
                      <div
                        className="w-[11px] h-[11px] rounded-full relative z-10"
                        style={{
                          background: done || current ? '#c9a96e' : 'transparent',
                          border: done || current ? 'none' : '1px solid rgba(201,169,110,0.2)',
                          boxShadow: current ? '0 0 10px rgba(201,169,110,0.5)' : 'none',
                        }}
                      />
                    </div>
                    <span
                      className="ob-sans uppercase text-[9px] tracking-[0.18em]"
                      style={{ color: done || current ? (current ? '#f5f0e8' : '#c9a96e') : '#8a7a65' }}
                    >
                      {step}
                    </span>
                  </div>
                );
              })}
            </div>
          </div>

          {/* ── Two-column ───────────────────────────────────────────── */}
          <div className="grid grid-cols-12 gap-12">
            <div className="col-span-7">
              {/* Progress */}
              <div className="p-8 mb-8" style={{ border: '1px solid rgba(201,169,110,0.12)', background: 'rgba(201,169,110,0.02)' }}>
                <div className="flex items-end justify-between mb-5">
                  <div>
                    <div className="ob-serif text-6xl" style={{ color: '#c9a96e' }}>72%</div>
                    <p className="ob-sans text-[9px] tracking-[0.18em] uppercase mt-1" style={{ color: '#8a7a65' }}>Ready to celebrate</p>
                  </div>
                  <p className="ob-sans text-sm font-light" style={{ color: '#8a7a65' }}>24 / 30 confirmed</p>
                </div>
                <div className="h-px overflow-hidden" style={{ background: 'rgba(201,169,110,0.1)' }}>
                  <div className="h-px" style={{ width: '72%', background: '#c9a96e' }} />
                </div>
              </div>

              {/* Next steps */}
              <div>
                <p className="ob-sans uppercase text-[10px] tracking-[0.22em] mb-5" style={{ color: '#8a7a65' }}>Next steps</p>
                {['Confirm final headcount', 'Share questionnaire link', 'Review venue proposal'].map((s, i) => (
                  <div
                    key={i}
                    className="ob-sans flex items-center gap-4 py-3.5"
                    style={{ borderBottom: '1px solid rgba(201,169,110,0.08)' }}
                  >
                    <div
                      className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                      style={{ border: '1px solid rgba(201,169,110,0.4)', background: i === 0 ? '#c9a96e' : 'transparent' }}
                    />
                    <span className="text-sm font-light" style={{ color: i === 0 ? '#c9a96e' : '#f5f0e8' }}>{s}</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="col-span-5">
              <div className="p-8" style={{ border: '1px solid rgba(201,169,110,0.15)', background: 'rgba(201,169,110,0.02)' }}>
                <h3 className="ob-serif italic text-xl mb-6" style={{ color: '#f5f0e8' }}>Ready to build your perfect day?</h3>
                <button
                  className="ob-sans w-full py-4 px-5 flex justify-between items-center"
                  style={{ background: 'rgba(201,169,110,0.05)', border: '1px solid rgba(201,169,110,0.3)' }}
                >
                  <span className="uppercase text-[10px] tracking-[0.2em]" style={{ color: '#c9a96e' }}>Chat with your concierge</span>
                  <span style={{ color: '#c9a96e', fontSize: '16px', letterSpacing: '-0.08em' }}>→</span>
                </button>

                <div className="mt-8 space-y-4">
                  {['Guests (24 confirmed)', 'Choose a plan', 'Send questionnaire'].map((item, i) => (
                    <div key={i} className="flex items-center gap-3">
                      <div
                        className="w-[5px] h-[5px] rounded-full flex-shrink-0"
                        style={{ background: i < 2 ? '#c9a96e' : 'transparent', border: i < 2 ? 'none' : '1px solid rgba(138,122,101,0.5)' }}
                      />
                      <span className="ob-sans text-sm font-light" style={{ color: i < 2 ? '#f5f0e8' : '#8a7a65' }}>{item}</span>
                    </div>
                  ))}
                </div>
              </div>
            </div>
          </div>
        </section>
      </div>
    </div>
  );
}
